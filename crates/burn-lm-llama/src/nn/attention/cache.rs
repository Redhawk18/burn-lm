...
impl<const D: usize, B: Backend> AutoregressiveCache<B, D> {
    ...
    pub fn append(&mut self, tokens: Tensor<B, D>) -> Tensor<B, D> {
        ...
        let seq_len_input = shape.dims::<D>()[self.seq_dim];

        let new_seq_len = self.cur_seq_len + seq_len_input;

        let mut indices_added_tokens = Vec::with_capacity(shape.dims::<D>().len());
        let mut indices_output = Vec::with_capacity(shape.dims::<D>().len());

        for (i, shape) in shape.dims::<D>().iter().enumerate() {
            if i == self.seq_dim {
    ...
    fn shrink(&mut self, num_removed: usize) {
        ...
        let mut slices_prev = Vec::with_capacity(shape.dims::<D>().len());
        let mut slices_curr = Vec::with_capacity(shape.dims::<D>().len());
